...
System::~System()
{
	Assert(mpD3DDevice == nullptr, "[System] Terminate() must be called to clean up!");
}

//----------------------------------------------------------------------------------------------------

void System::Initialize(HWND window, bool fullscreen)
{
	...
	Assert(SUCCEEDED(hr), "[System] Failed to create device or swap chain.");
	...
	Assert(SUCCEEDED(hr), "[System] Failed to access swap chain buffer.");
	...
	ProperlyRelease(pBackBuffer);
	Assert(SUCCEEDED(hr), "[System] Failed to create render target view.");
	...
	Assert(SUCCEEDED(hr), "[System] Failed to create depth stencil buffer.");
	...
	Assert(SUCCEEDED(hr), "[System] Failed to create depth stencil view.");
...
void System::Terminate()
{
	...
	ProperlyRelease(mpSwapChain);
	ProperlyRelease(mpImmediateContext);
	ProperlyRelease(mpD3DDevice);
}
